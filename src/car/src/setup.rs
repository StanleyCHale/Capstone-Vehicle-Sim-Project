...
pub fn simulation_setup(app: &mut App) {
    ...
    .add_systems(Update, (user_control_system,).run_if(in_state(GameState::InGame)));
}
...
pub fn camera_setup(app: &mut App) {
    ...
    .add_systems(Update, (camera_az_el::az_el_camera, camera_parent_system).run_if(in_state(GameState::InGame))); // setup the camera
}
